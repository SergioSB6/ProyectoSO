...
void *AtenderCliente (void *socket)
{
	...
	while (terminar ==0)
	{
		...
		{		
			...
			//Envio la comfirmación al jugador que ha enviado la invitación
			printf("He enviado una invitación a %s", usuario);
			char resp[20];
			sprintf(resp,"8/");
			strcpy(respuesta, resp);
			
			//Envio invitación al socket del invitado
			char resp1[20];
		...
		{				
			...
			{				
				...
				if(indiceSala != -1)
				{
					//printf("Pongo el invitado en la sala del usuario");
				...
				else
				{
					...
					//printf("Lista salas num despues de crear sala: %d \n", listaSalas.num);
					//printf("Numero de gente dentro de la sala %d \n" ,listaSalas.salas[listaSalas.num - 1].num);
				}
		 ...
		 else if(codigo==8)
		{
			...
			printf("Numero de gente en la sala: %d", listaSalas.salas[indice].num);
